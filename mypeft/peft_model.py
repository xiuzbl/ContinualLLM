...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    @classmethod
    def from_pretrained(cls, model, model_id, adapter_name="default", is_trainable=False, replace_lora_weights=False, **kwargs):
        r"""
        ...
        """
        print('Load from pre-trained model weights', flush=True)
        from .mapping import MODEL_TYPE_TO_PEFT_MODEL_MAPPING, PEFT_TYPE_TO_CONFIG_MAPPING
        # pdb.set_trace()

        if replace_lora_weights:
            lora_path = os.path.join(model_id, 'adapter_model.bin')
            lora_weights = torch.load(lora_path, map_location=torch.device("cuda" if torch.cuda.is_available() else "cpu"))
            replace_model_weights_with_lora(model, lora_weights)
            return model

        else:
            # load the config
            print(f'Directly use the trained LoRA module.', flush=True)
    ...
    def load_adapter(self, model_id, adapter_name, is_trainable=False, **kwargs):
        ...
        print(f'Load the LoRA weights into the model', flush=True)
...
def replace_model_weights_with_lora(model, lora_weights):

    print(f"Replace model weights with lora's trained weights")
    ...
    for key, value in state_dict.items():
        if re.fullmatch(lora_modules, key) and 'lora' not in key:
            # if 'q_proj' in key or 'v_proj' in key: #chainyo
            if 'q_proj' in key or 'v_proj' in key or 'k_proj' in key or 'o_proj' in key: #tloen
                # pdb.set_trace()
                lorakey = 'base_model.model.'+key[:-7]
                loraA = lora_weights[lorakey+'.lora_A.weight']
                loraB = lora_weights[lorakey+'.lora_B.weight']
                new_weights = value + torch.matmul(loraB, loraA) 
                # new_weights = value # test whether the updated weights are different from the original backbone model performance
                # print(f'value: {value[0]}', flush=True)
                # print(f'new weight: {new_weights[0]}', flush=True)
